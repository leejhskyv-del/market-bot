...
# ==========================================
# 🧠 AI 심층 분석 엔진 (전문가 페르소나 복구)
# ==========================================
def get_expert_ai_analysis(news, market_summary):
    prompt = f"""너는 월스트리트 20년 경력의 수석 매크로 전략가다. 
지표({market_summary})와 뉴스({news}) 사이의 숨은 상관관계를 해석하여 시스템 리스크를 진단하라.
1. 단순 요약은 금지한다. 지수 하락 중 환율/VIX 폭등 시 블랙스완 징후로 판단하라.
2. 200일선 아래의 반등 뉴스는 '데드캣 바운스'로 의심하라.
3. JSON 형식 엄수: {{"score": int(-2~2), "summary": "시장국면", "risk": "핵심리스크2가지", "strategy": "투자행동가이드"}}"""
    try:
...
def calc_master_score(spy, qqq, fx_data, vix, dxy, ai_score):
    ...
    log(f"SCORE_LOG: SPY_G={gap(spy[0],spy[2]):.1f}, FX_1YG={gap(fx_c,fx_1y):.1f}, VIX={vix}, AI={ai_score}")
...
# ==========================================
def main():
    log(f"📊 퀀텀 인사이트 가동 시작 ({datetime.now()})")
    ...
    news_context = "\n".join([f"▶ {e.title}: {getattr(e, 'summary', '')[:100]}" for e in news_feed.entries[:5]])
    ...
    status_icon = "✅ 정상" if spy[0] > 0 else "⚠️ 데이터 지연 (최신화 대기 중)"
