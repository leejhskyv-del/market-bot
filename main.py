...
2. 현재가가 200일선 '위'인지 '아래'인지 데이터(market_summary)를 보고 추세를 판단하라.
3. 200일선 아래의 호재는 데드캣 바운스로 의심하라.
4. JSON 형식 엄수: {{"score": int(-2~2), "summary": "시장국면", "risk": "핵심리스크2가지", "strategy": "대응가이드"}}"""
    try:
...
def main():
    ...
    fx_data = get_fx_final(); gold = get_gold_full()
    ...
    # [핵심 수정] AI에게 지수가 200일선 대비 얼마나 위에 있는지 정보를 넘겨줌
    market_info = f"S&P500:{spy[0]}(200일선대비 {gap(spy[0], spy[2]):+.1f}%), VIX:{vix}, 환율:{fx_data[0]}"
    ai = get_expert_ai_analysis(news_context, market_info)
    ...
    spy_str = format_idx_msg(spy); qqq_str = format_idx_msg(qqq); kos_str = format_idx_msg(kospi)
    ...
    fx_status = "⚠️ 역사적 고점" if fx_gap_2y > 8 else ("🟠 환율 높음 (주의)" if fx_gap_2y > 4 else "✅ 정상범위")
